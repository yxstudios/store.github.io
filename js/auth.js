...
supabase.auth.onAuthStateChange(async function(event, session) {
    ...
    if (event === 'SIGNED_IN' && session) {
        ...
        console.log('Provider:', provider, 'Metadata:', metadata);
        
        if (provider === 'discord' || (metadata.iss && metadata.iss.includes('discord'))) {
            console.log('Guardando Discord...');
            await supabase.from('profiles').upsert({
                id: user.id, full_name: metadata.full_name || metadata.name || '',
                discord_id: metadata.provider_id || '', discord_username: metadata.full_name || metadata.name || '',
                discord_avatar: metadata.avatar_url || metadata.picture || '', discord_linked_at: new Date().toISOString(),
                avatar_url: metadata.avatar_url || metadata.picture || '', updated_at: new Date().toISOString()
            });
        }
        
        if (provider === 'spotify' || (metadata.iss && metadata.iss.includes('spotify'))) {
            console.log('Guardando Spotify...');
            var spotifyName = metadata.full_name || metadata.name || 'Usuario Spotify';
            var spotifyAvatar = metadata.avatar_url || metadata.picture || '';
            var spotifyEmail = metadata.email || user.email || '';
            var spotifyPlan = 'Free'; var spotifyFollowers = 0;
            if (metadata.data) { spotifyPlan = metadata.data.product || 'Free'; spotifyFollowers = metadata.data.followers?.total || 0; }
            var spotifyToken = session.provider_token; var playlistsCount = 0; var artistsCount = 0;
            if (spotifyToken) {
                try { var pr = await fetch('https://api.spotify.com/v1/me/playlists?limit=1', { headers: { 'Authorization': 'Bearer ' + spotifyToken } }); if (pr.ok) { var pd = await pr.json(); playlistsCount = pd.total || 0; } } catch (e) {}
                try { var ar = await fetch('https://api.spotify.com/v1/me/following?type=artist&limit=1', { headers: { 'Authorization': 'Bearer ' + spotifyToken } }); if (ar.ok) { var ad = await ar.json(); artistsCount = ad.artists?.total || 0; } } catch (e) {}
            }
            await supabase.from('profiles').upsert({
                id: user.id, spotify_id: metadata.provider_id || user.id, spotify_name: spotifyName,
                spotify_avatar: spotifyAvatar, spotify_email: spotifyEmail,
                spotify_url: metadata.data?.external_urls?.spotify || '', spotify_plan: spotifyPlan,
                spotify_followers: spotifyFollowers, spotify_playlists: playlistsCount, spotify_artists: artistsCount,
                spotify_linked_at: new Date().toISOString(), avatar_url: spotifyAvatar || metadata.avatar_url, updated_at: new Date().toISOString()
            });
