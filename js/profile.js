...
function setupNavigation() {
    document.querySelectorAll('.profile-nav-link').forEach(function(link) {
        link.addEventListener('click', function(e) {
            ...
            document.querySelectorAll('.profile-nav-link').forEach(function(l) {
...
function setupEventListeners() {
    ...
    document.getElementById('changePasswordBtn')?.addEventListener('click', async function() {
        ...
        if (newPass.length < 6) {
            showNotification('Contraseña débil', 'Mínimo 6 caracteres', 'error');
            return;
        }
    ...
    document.getElementById('changeEmailBtn')?.addEventListener('click', async function() {
        ...
        if (!newEmail) {
            showNotification('Campo vacío', 'Ingresa un nuevo correo', 'error');
            return;
        }
        ...
        if (error) {
            ...
            showNotification('Solicitud enviada', 'Revisa tu nuevo correo para confirmar', 'success');
        }
    });
    ...
    document.getElementById('avatarUpload')?.addEventListener('change', async function(e) {
        ...
        // Convertir a base64 para guardar localmente
    ...
    document.querySelectorAll('.accent-color-btn').forEach(function(btn) {
        btn.addEventListener('click', function() {
            ...
            document.querySelectorAll('.accent-color-btn').forEach(function(b) { b.classList.remove('active'); });
            this.classList.add('active');
        });
    });
...
function loadPurchases() {
    ...
    if (!orders.length) {
        tbody.innerHTML = '<tr><td colspan="6" class="text-muted">No tienes compras realizadas</td></tr>';
        return;
    }
    
    tbody.innerHTML = orders.reverse().map(function(order, index) {
        ...
        <tr>
            ...
            <td><span class="status-badge status-${order.status || 'completed'}">${order.status || 'Completado'}</span></td>
            <td>
                <button class="btn-sm btn-outline" onclick="downloadInvoice('${order.id}')">
...
function loadInvoices() {
    ...
    if (!orders.length) {
        list.innerHTML = '<p class="text-muted">No hay facturas disponibles</p>';
        return;
    }
    ...
    list.innerHTML = orders.map(function(order) {
        ...
        <div class="invoice-card">
            <div class="invoice-info">
                ...
                <div>
                    <strong>Factura #${(order.id || '').substring(0, 8)}</strong>
                    <p>${new Date(order.date).toLocaleDateString()} - $${order.total ? order.total.toFixed(2) : '0.00'}</p>
                </div>
            </div>
            <button class="btn-sm btn-outline" onclick="downloadInvoice('${order.id}')">
...
// ============================================
// DESCARGAR FACTURA (Simulación PDF)
// ============================================
window.downloadInvoice = function(orderId) {
    ...
    var content = `
        ...
        Productos: ${order.items ? order.items.map(function(i) { return i.name + ' x' + (i.quantity || 1); }).join(', ') : 'N/A'}
        ========================
        Gracias por tu compra!
    `;
    
    // Descargar como archivo de texto (simulación)
